...
import {
  FiRadio, FiServer, FiShield, FiGlobe, FiUsers, FiVideo, FiPhoneCall, FiCpu, FiPlay,
...
import { AI_DECISION_STATUS_META } from './ai/AIDecisionsPanel';
import SlideOver from './common/SlideOver';

type TabId = 'overview' | 'metrics' | 'connections' | 'ai' | 'history';
...
const StatTile: React.FC<{ label: string; value: string }> = ({ label, value }) => (
  <div className="stat-tile" style={{ padding: '12px 14px' }}>
    <div className="label-caps-sm" style={{ letterSpacing: 0, marginBottom: 5 }}>{label}</div>
    <div className="metric-value" style={{ fontSize: '18px' }}>{value}</div>
  </div>
);

const EmptyTabState: React.FC<{ text: string }> = ({ text }) => (
  <div className="flex items-center justify-center" style={{ padding: '32px 0', color: 'var(--text-muted)', fontSize: '11px' }}>
    {text}
  </div>
);
...
/**
 * Large right-side overlay opened on node selection — replaces the old always-inline detail
 * panel so the sidebar can stay compact. Visibility is fully derived from `selectedNode`, so
 * closing (X / Escape / backdrop) is just `setSelectedNode(null)`, identical to the pre-existing
 * "click canvas background to deselect" behavior.
 */
const NodeInspector: React.FC = () => {
  const { selectedNode, state, setSelectedNode, events, aiDecisions } = useSimulation();
    ...
    if (type === 'tower') {
      ...
      return (
        <>
          ...
          {traffic && (
            <>
              <div className="label-caps-sm" style={{ marginTop: '16px', marginBottom: '8px' }}>
                Traffic Breakdown
              </div>
    ...
    <SlideOver
      ...
      title={
        <div className="flex items-center gap-2.5">
          ...
          {meta && <span style={{ marginLeft: 10 }}><StatusBadge status={meta.status} /></span>}
        </div>
      }
    >
      {/* Tab bar */}
      <div className="node-tab-bar" style={{ padding: '0 20px' }}>
        {TABS.map((tab) => (
          <button
            key={tab.id}
          ...
          >
            ...
            {tab.id === 'history' && nodeAlerts.length > 0 ? ` (${nodeAlerts.length})` : ''}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: '20px 24px' }}>
        <AnimatePresence mode="wait">
          ...
          >
                ...
                <div className="flex flex-wrap gap-2">
                  {connectedNodes.map((n) => (
                    <button
                      ...
                      style={{
                        padding: '5px 11px', borderRadius: '7px', background: 'rgba(59, 130, 246, 0.12)',
                        border: '1px solid rgba(59, 130, 246, 0.2)', color: 'var(--accent-blue)', fontSize: '12px',
            ...
            {activeTab === 'ai' && (
              <>
                <div className="flex items-center gap-1.5" style={{ marginBottom: 14, color: 'var(--text-muted)', fontSize: '11px' }}>
                  <FiZap style={{ width: 12, height: 12 }} />
                  Network-wide AI decisions — the backend's AI loop simulates a single entity, not per-node
                </div>
                {aiDecisions.length > 0 ? (
                  <div className="flex flex-col" style={{ gap: 2 }}>
                    {aiDecisions.map((d) => {
                      const dmeta = AI_DECISION_STATUS_META[d.status];
                      return (
                        <div key={d.id} className="flex items-center justify-between gap-2" style={{ padding: '9px 0', borderBottom: '1px solid rgba(148, 163, 184, 0.06)' }}>
                          <div className="flex items-center gap-2.5" style={{ minWidth: 0 }}>
                            {dmeta.dot ? (
                              <span className={`indicator-dot ${dmeta.dot}`} style={{ flexShrink: 0 }} />
                            ) : (
                              <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#475569', flexShrink: 0 }} />
                            )}
                            <span style={{ fontSize: '12px', color: 'var(--text-secondary)', lineHeight: 1.3 }}>{d.text}</span>
                          </div>
                          <span style={{
                            fontSize: '9px', fontWeight: 700, color: dmeta.color, background: `${dmeta.color}1a`,
                            border: `1px solid ${dmeta.color}33`, padding: '2px 7px', borderRadius: '9px',
                ...
                ) : <EmptyTabState text="Waiting for the first AI decision cycle…" />}
              </>
            )}
            {activeTab === 'history' && (
              nodeAlerts.length > 0 ? (
                <div className="flex flex-col gap-3">
                  {nodeAlerts.map((a) => (
                    <div key={a.id} className="flex items-start gap-2">
                      ...
                      <div>
                        <span style={{ fontSize: '12px', color: 'var(--text-secondary)', lineHeight: 1.4 }}>{a.message}</span>
                        <div style={{ fontSize: '10px', color: 'var(--text-muted)', marginTop: '2px', fontFamily: 'monospace' }}>{a.timestamp}</div>
                      </div>
                    </div>
                  ))}
                </div>
