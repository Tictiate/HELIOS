...
class EmergencyTrafficSurge(NetworkScenario):
    name = "Emergency Traffic Surge"
    
    def apply(self, snapshot: Dict[str, Any]) -> Dict[str, Any]:
        state = copy.deepcopy(snapshot)
        # Dramatically increase emergency users which will spike the demand for the CRITICAL slice
        state["emergency_users"] = int(state.get("users", 100) * random.uniform(0.3, 0.5))
        # Total users also slightly increases
        state["users"] = int(state.get("users", 100) * random.uniform(1.1, 1.3))
        # Wait, if we just change emergency_users here, generate_mock_telemetry overrides it?
        # No, the scenario applies ON TOP of the snapshot generated by generate_mock_telemetry.
        # But wait, slices are already calculated in generate_mock_telemetry based on emergency_users.
        # If we change emergency_users here, we need to re-calculate slice demand!
        # Let's re-calculate it.
        demand_factor = 0.5
        new_demand = state["emergency_users"] * demand_factor
        
        slices = state.get("slices", [])
        for s in slices:
            if s["name"] == "Emergency":
                s["active_users"] = state["emergency_users"]
                s["current_demand_mbps"] = round(new_demand, 2)
                
                # Recompute SLA and latency based on current allocation
                alloc = s["allocated_bandwidth_mbps"]
                ratio = new_demand / alloc if alloc > 0 else 999.0
                
                if ratio <= 0.8:
                    lat = s["latency_target_ms"] * 0.5
                    pl = s["packet_loss_target_pct"] * 0.1
                elif ratio <= 1.0:
                    lat = s["latency_target_ms"] * ratio
                    pl = s["packet_loss_target_pct"] * ratio
                else:
                    lat = s["latency_target_ms"] * (ratio ** 2)
                    pl = s["packet_loss_target_pct"] * (ratio ** 2)
                    
                s["current_latency_ms"] = round(lat, 2)
                s["current_packet_loss_pct"] = round(min(100.0, pl), 2)
                
                # SLA
                if lat > s["latency_target_ms"] * 1.5 or pl > s["packet_loss_target_pct"] * 1.5:
                    s["status"] = "VIOLATION"
                elif lat > s["latency_target_ms"] or pl > s["packet_loss_target_pct"]:
                    s["status"] = "DEGRADED"
                else:
                    s["status"] = "HEALTHY"
...
AVAILABLE_SCENARIOS = {
    "Emergency Traffic Surge": EmergencyTrafficSurge(),
